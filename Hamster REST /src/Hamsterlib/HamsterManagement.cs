...
{
    public class HamsterManagement : IHamsterManagement
    {
        internal const int MaxNameLength = 31;
        private const int Rpm = 25;
        internal const ushort BasePrice = 17;
        private const ushort StayPrice = 5;
        private const ushort FeedPrice = 2;
        private const ushort CarePrice = 1;

        private static Random _feedFailRandom = new Random(42);


        public static double FeedSuccessProbability { get; set; } = 0.05;
        ...
        public int NewHamster(string ownerName, string hamsterName, ushort treats)
        {
        ...
        public ushort ReadEntry(int id, out string ownerName, out string hamsterName, out ushort price)
        {
            var entry = _dataStore.GetEntryById(id);
        ...
        public ushort GiveTreats(int id, ushort treats)
        {
            var entry = _dataStore.GetEntryById(id);
            var newTreats = entry.Treats - treats;

            if (_feedFailRandom.NextDouble() < FeedSuccessProbability)
            {
            ...
            if (newTreats < 0)
            {
                entry.Price += (ushort)(-newTreats * FeedPrice);
                entry.Treats = 0;
            }
            else
            {
                entry.Treats = (ushort)newTreats;
            }
            _dataStore.UpdateEntry(entry);
            return entry.Treats;
        }
        ...
        /// <exception cref="HamsterNameTooLongException"></exception>
        public ushort Collect(string ownerName)
        {
