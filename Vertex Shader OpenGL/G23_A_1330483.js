...
window.onload = function init() {
    ...
    document.getElementById('threshold1').addEventListener('input', function(event) {
        var threshold1 = parseFloat(event.target.value);
        document.getElementById('threshold1Val').textContent = threshold1;
        gl.uniform1f(gl.getUniformLocation(program, "uThreshold1"), threshold1);
    });

    document.getElementById('threshold2').addEventListener('input', function(event) {
        var threshold2 = parseFloat(event.target.value);
        document.getElementById('threshold2Val').textContent = threshold2;
        gl.uniform1f(gl.getUniformLocation(program, "uThreshold2"), threshold2);
    });

    document.getElementById('threshold3').addEventListener('input', function(event) {
        var ambientIntensity = parseFloat(event.target.value);
        document.getElementById('ambientIntensityVal').textContent = threshold3;
        gl.uniform1f(gl.getUniformLocation(program, "uAmbientIntensity"), ambientIntensity);
    });

    document.getElementById('shininess1').addEventListener('input', function(event) {
        var shininess = parseFloat(event.target.value);
        document.getElementById('shininessVal').textContent = shininess1;
